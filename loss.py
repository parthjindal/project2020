...
class FKDLoss():
    ...
    def forward(self, input: Tensor, soft_target: Tensor, labels: Tensor) -> Tensor:
        ...
        return F.kl_div(F.log_softmax(input/self.T, dim=1), F.softmax(soft_target/self.T, dim=1), reduction='batchmean') + F.cross_entropy(F.softmax(input,dim=1), labels, reduction='mean')
    __call__ = forward
